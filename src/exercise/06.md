...
- Use react-error-boundary's ErrorBoundary wrapper component to handle errors
  more succinctly
  - Is generic; receives a specific function component in the FallbackComponent
    prop
- Problem: how to reset after an error?
  - **Solution 1**: We could add a unique key prop when using the error boundary
    component, to unmount it / reset its error state, so we can exit the error
    and access the child again
    - In general, a unique key prop is a good trick to reset the state of a
      component
    - However, this also remounts the child.
  - **Solution 2** (better): use resetErrorBoundary function in a try again
    button for example this function is already passed as a prop to the
    FallbackComponent
    - Then in combination, use the "onReset" prop on the ErrorBoundary component
      as needed (e.g. to reset parent UI state for the query input)
  - **Solution 3** (better): just use one prop on ErrorBoundary: resetKeys,
    which takes an array of values on change of which to reset the error
    - No try again button needed, less props needed
  - A combination of Solutions 2 & 3 is probably best. Gives the user most
    flexibility
- Consider using a status instead of returning based on isLoading / isError
  booleans
  - Saves some state resetting
  - Can set the initial status to idle', or 'pending' if props will trigger the
    request as soon as the component mounts (this can be important in
    ErrorBoundary remounting use cases)
- Consider moving similar state into one state object so that can just set it
  once
  - Just in case there are unnecessary and risky extra rerenders through
    multiple state settings (but this is not usually a problem)
  - useReducer is probably a better solution to this problem anyway
- For errors, add the prop role="alert" to the element
  - It's for screenreaders
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=alexblyth%40pm.me).
