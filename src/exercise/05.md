...
- Refs are a way to attach references to DOM nodes that are rendered on the page
- But we should use useRef anytime we want to maintain a reference to something
  and be able to mutate it without triggering a rerender
  - useState is normally preferable, if we don't mind about triggering rerenders
- JSX does not create/render DOM nodes, it creates UI Descriptor Objects (React
  Elements)
- Those ultimately get compiled to JavaScript when the return function is called
  - and then rendered as DOM nodes to the page
- Anytime we want to interact directly with the DOM, it would be a side effect
  - So we would be using useEffect for that
  - We could use also then use direct javascript, e.g. classname query
  - But probably it's easier to use useRef instead
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=05%3A%20useRef%20and%20useEffect%3A%20DOM%20interaction&em=alexblyth%40pm.me).
