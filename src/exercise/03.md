...
- Three stages of component lifecycle: mount, update, unmount
- Updates are caused by a parent re–render, state change, or context change
- Can pass colors as strings, e.g. 'color: MediumSpringGreen', as second
  argument of console.logs – but must preprend "%c" to first argument string
- State should always be kepts closest to the component where it is most
  relevant
  - Preferably inside that component (colocated state), unless it needs to be
    shared with other sibling or parent components
  - In which case it should be lifted to the least common parent in the tree
  - But colocating state wherever possible is best practice (maintainability =>
    less worrying about props when using components, performance => lower amount
    of renders)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=03%3A%20Lifting%20state&em=alexblyth%40pm.me).
