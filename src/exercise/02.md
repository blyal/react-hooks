...
- Always use a function update when calculating a new state based on previous
  state (e.g. setState(previousState => previousState + 1). Exception maybe with
  useReducer.)
- Use a function as initial state (lazy state initialization) is the initial
  state is something computationally expensive or takes time (e.g. complicated
  function or localStorage). Kent does this maybe 2% of the time. Initial state
  is then only calculated at first render
  - Lazy initializers are functions passed to useState and useReducer
- Customs hooks are just functions that use hooks (built–in or other custom
  hooks)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=alexblyth%40pm.me).
