...
- Managed State vs Derived State
  - Managed State: state that you explicitly manage
  - Derived State: state that you can calculate based on other state
  - Derived State looks like const regularVariable = function call to calculate
    the state
  - So whenever a rerender is triggered due to a change in managed state, the
    derived state is automatically calculated (calculated on every render)
  - Can instead have all managed state and a function to update and sync all
    states whenever state needs to be updated
  - But this is unnecessary, can be messy, and is risky. Derived state is
    preferable
  - Can also derive state from props
- Use double pipe || in initial useState instead of ternary
  - Double pipe uses the right value if the left is falsy
  - Versus double question mark, which uses right value if left is undefined or
    null
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=04%3A%20useState%3A%20tic%20tac%20toe&em=alexblyth%40pm.me).
